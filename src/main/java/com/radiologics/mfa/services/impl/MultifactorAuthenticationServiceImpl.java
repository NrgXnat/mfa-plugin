...
public class MultifactorAuthenticationServiceImpl extends AbstractHibernateEntityService<MultifactorEntity, MultifactorDAO> implements MultifactorAuthenticationService {
	...
	public MultifactorAuthenticationServiceImpl(final SiteConfigPreferences siteConfigPreferences,
												final MFAPreferencesHelper mfaPreferencesHelper,
												final UserRegistrationDataService userRegistrationDataService){
		this.siteConfigPreferences       = siteConfigPreferences;
		this.mfaPreferencesHelper        = mfaPreferencesHelper;
		this.userRegistrationDataService = userRegistrationDataService;
	}

	public List getAllMultifactorEntities() {
	...
	public MFAStrategyI getPreferredMFAStrategy(MultifactorEntity mfe) throws MFAStrategyNotFoundException {
		String preferredMFA =  mfaPreferencesHelper.getPreferredMFAMethod();
		if (null != mfe) {
			preferredMFA =  mfe.getPreferredMfas(); //User preferred MFA
		}
		if (null == preferredMFA) {
			preferredMFA =  mfaPreferencesHelper.getPreferredMFAMethod();
		}
	...
	public String getPreferredMFAMethod(MultifactorEntity mfe) {
		String userOveriddenPreferredMethod = mfaPreferencesHelper.getPreferredMFAMethod();
		if (null == mfe) {
		...
		if (null == userOveriddenPreferredMethod) {
			...
			userOveriddenPreferredMethod = mfaPreferencesHelper.getPreferredMFAMethod();
		}
		return userOveriddenPreferredMethod;
	}
	...
	public MfaModel toPojo(MultifactorEntity mfe) {
		try {
			...
			if(!mfe.isMfaRegistered() && mfaStrategy.needsRegistration() && mfaStrategy instanceof GoogleAuthenticatorStrategy){
				model.setSecret(mfe.getAuthenticatorSecret());
	...
	public Boolean isMFARequired(String username){
		return ((BooleanUtils.toBooleanDefaultIfNull(siteConfigPreferences.getBooleanValue("requireMfa"),false) &&
				!Roles.isSiteAdmin(username)) ||
				(BooleanUtils.toBooleanDefaultIfNull(siteConfigPreferences.getBooleanValue("requireAdminMfa"),false) &&
						Roles.isSiteAdmin(username)));
	}
}
